 ...
 *     Christoph Christoph Läubrich - adding helper for getting a JarFileSystem
 *******************************************************************************/
...
public class JRTUtil {
	...
	// ANDROID-HACK: detect Android runtime so we can bypass all jrt-fs logic
	public static boolean isAndroid() {
		String vmName = System.getProperty("java.vm.name", ""); //$NON-NLS-1$ //$NON-NLS-2$
		return vmName.contains("ART") || vmName.contains("Dalvik"); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	public static JrtFileSystem getJrtSystem(File image, String release) throws IOException {
		// ANDROID-HACK: jrt file system provider does not exist on Android
		if (isAndroid()) {
			throw new IOException("JRT file system is not available on Android"); //$NON-NLS-1$
		}
